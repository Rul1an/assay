...
**Goal**: `from assay_sdk import Evaluator` becomes the standard UX.

## Phase 0: API Contract & DX (P0)
- [ ] **0.1 Evaluator Constructor**
    - [ ] Implement `Evaluator(config=...)` loader logic (`assay_sdk/evaluator.py`).
    - [ ] Implement `EvaluatorOptions` dataclass.
    - [ ] Verify `Evaluator()` defaults to `eval.yaml` in cwd.

## Phase 1: Result Types & Errors (P0)
- [ ] **1.1 Result Objects**
    - [ ] Create `assay_sdk/result.py` (`EvalRun`, `CompareResult`, `TestResult`).
    - [ ] Implement helpers: `__bool__`, `raise_for_status()`, `to_github_summary()`.
- [ ] **1.2 Typed Errors**
    - [ ] Create `assay_sdk/errors.py` (`AssayError`, `ConfigError`, `RegressionError`).

## Phase 2: Baselines & Compare (P0)
- [ ] **2.1 BaselineStore**
    - [ ] Create `assay_sdk/baseline.py` handling `.eval/baselines/`.
...
- [ ] **3.2 Hardened Prompts**
    - [ ] Write `faithfulness_v1.md` and `relevance_v1.md` in `assay-metrics`.
- [ ] **3.3 Caching**
...
- [ ] **4.2 Migration**
    - [ ] Create `assay_sdk/compat.py` for CLI config loading.
