...
- Status: Active (`T1a`, `T1b`, `G3`, `P2a`, and `H1` are public in `v3.3.0`; `G4-A`, `P2c`, and `K1-A` Phase 1 are public in `v3.4.0`; `K2-A` Phase 1 is now implemented on `main` as the current bounded MCP authorization-discovery slice, but not yet public in a tagged release)
- Date: 2026-03-23
...
7. **`K2` — MCP authorization-discovery evidence** (current bounded evidence wave after `K1-A`; evidence-first; no pack in the same slice). `K2-A` Phase 1 is now implemented on `main` as a bounded MCP import seam, but not yet public in a tagged release. See [PLAN-K2](PLAN-K2-MCP-AUTHORIZATION-DISCOVERY-EVIDENCE-2026q2.md), [K2-A formal freeze](K2-A-PHASE1-FREEZE.md), and [K2-A freeze prep](K2-A-PHASE1-FREEZE-PREP.md)
