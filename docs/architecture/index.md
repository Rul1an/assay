...
- [PLAN — P9b Pydantic Reduced Case-Result Evidence Recut (Q2 2026)](./PLAN-P9B-PYDANTIC-REPORTCASE-RESULT-EVIDENCE-2026q2.md) — next selected Pydantic slice: recut the old report-wrapper sample around one reduced case-result artifact derived from `EvaluationReport.cases[]`, possible importer-only support only if the live recut succeeds, not a Trust Basis claim or public receipt family
