...
| 1              | 2026-08  | `E_EVIDENCE_LIMIT_EXCEEDED` and `E_EVIDENCE_PATH_REJECTED` leave §5.4: the privileged-mcp-action verifier constructs both for typed `Limit*` and `SecurityPathTraversal` findings (#2165). `SecurityAbsolutePath` remains enum-only and is not required for that command's completeness. |
