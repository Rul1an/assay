...
| E_EVIDENCE_LIMIT_EXCEEDED | `ReasonCode::EEvidenceLimitExceeded` | Registered in §5.1. Nothing constructs the variant. Temporary: #2165 is the intended first consumer. |
| E_EVIDENCE_PATH_REJECTED | `ReasonCode::EEvidencePathRejected` | Registered in §5.1. Nothing constructs the variant. Temporary: #2165 is the intended first consumer. |
...
| 1              | 2026-08  | Added `E_EVIDENCE_CONTRACT` (§5.1) for typed `ErrorClass::Contract` / `Contract*` format-contract defects. Additive under `reason_code_version` 1 on the same terms as `E_EVIDENCE_INTEGRITY`. Closes the stated gap that row previously pointed at #2219. Reserved under §5.4 until #2165 constructs it. |
