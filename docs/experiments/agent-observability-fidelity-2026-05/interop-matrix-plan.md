...
> OTel GenAI / OpenInference / Runner interoperability coverage matrix
> before any harness or delegated measurement work. It does not add
> schema sidecars, does not translate between vocabularies at runtime,
> does not rank products, and does not publish delegated findings.
>
...
Slice 5 does not add the `interop_coverage_cell.v0` schema. Slice 6
should add the sidecar only after this plan's row shape has survived
review.
...
| Cell id | Scenario / shape | Vocabulary profiles | Join key | Evidence layer | Purpose |
|---|---|---|---|---|---|
...
The first four cells can reuse Slice 4 synthetic fixtures. The fifth may
use a new synthetic fixture in Slice 6, but it must remain synthetic and
must not publish delegated measurements.

## Proposed Output Shape

Slice 6 should emit one `interop_coverage_cell.v0` row per matrix cell.
...
- No delegated runs are required or published in Slice 5 or the Slice 6
  starter harness.
- Slice 6 may add the `interop_coverage_cell.v0` schema sidecar only
