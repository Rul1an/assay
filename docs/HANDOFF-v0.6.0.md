...
We have successfully shipped **Assay SDK v0.6.0**, a hardening release that establishes "Implementation Parity" for modern Python stacks. This release transforms the SDK from a prototype adapter to a production-grade instrumentation library.

**Key Wins:**
1.  **Async/Await Parity**: Full support for `asyncio`, blocking loops, and streaming responses (`AsyncStreamAccumulator`).
2.  **Boilerplate Annihilation**: `assay-sdk` now replaces manual tracing loops.
...
### 1. Python SDK (`assay-sdk` v0.6.0)
*   **Async Support**: `assay_sdk.async_openai` module mirrors the sync API.
*   **Context Injection**: `assay_sdk.context` allows implicit `writer` passing (Opt-in).
...
*   **CI Hardening**: Added `cargo test --workspace` to `.github/workflows/assay.yml`.
...
| Area | Action | Impact |
| :--- | :--- | :--- |
| **Rust** | `cargo clippy` & Test Fixes | Fixed compilation error in `assay-core`; optimized string allocs. |
...
3.  **Next Release (v0.7.0)**:
    Focus on **Assay CLI** ergonomics (viewing traces locally) now that ingestion/recording is solved.
