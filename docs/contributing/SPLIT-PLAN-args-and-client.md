...
| assay-evidence json_strict | 970 LOC | ✅ Split into `json_strict/` (mod, scan, dupkeys, errors) |
| assay-registry canonicalize | 1155 LOC | ✅ Split into `canonicalize/` (mod, yaml, json, digest, errors, tests) |

---

## 2. Current >800 LOC inventory (reference)

*Generated by `./scripts/largest_rust_files.sh` on HEAD `48861ed2` (2026-02-13). Verify before each split.*
...
### 3F. API stability gate (required for library crates)

For crates that expose public API, run semver checks against `origin/main` before merge:
...
If API change is intentional, document it explicitly in PR and changelog.

### 3G. Feature matrix gate (required when crate has features)

`--no-default-features` alone is insufficient. Minimum:
...
### 3H. Parser/crypto hardening gates (required where applicable)

For parser/canonicalization/verification splits, add:

- Property tests: determinism and invariants (same input => same normalized output/code)
- Corpus tests: malformed inputs and edge cases
- Fuzz target (nightly/periodic CI is acceptable), no panics/UB

### 3I. Concurrency/model gates (required for stateful runtime/store code)

For `runner` and `mandate_store`-class code, require concurrency validation:

- Loom model tests for race-sensitive state transitions, or
- Kani/prover checks for critical invariants, plus
- deterministic scheduling tests for exit/status mapping

### 3J. No temporary panics in mergeable commits

No `todo!()` / `unimplemented!()` / placeholder panic paths in final split PR.
Enforce in clippy:
...
### 3K. Keep this document fresh

If the inventory in §2 diverges from `./scripts/largest_rust_files.sh`, update this plan in the same PR as the split.

---

## 4. Next priorities (with split designs)

### 4.1 verify.rs (1065) — high ROI
...
- Property tests for envelope normalization and algorithm handling
- Fuzz/corpus gate for malformed DSSE and signature payloads

**Move map:** See [Appendix B: verify move map](#appendix-b-verify-move-map).

---

### 4.2 runner.rs (1042) — medium/high ROI
...
- Loom/Kani or equivalent concurrency/model checks for scheduler/state transitions

**Move map:** See [Appendix C: runner move map](#appendix-c-runner-move-map).

---

### 4.3 mandate_store.rs (1046)
...
**Gates:** Roundtrip write→read invariants; concurrency/locking semantics (required, not optional); deterministic conflict resolution behavior.
...
### 4.7 lockfile.rs (863) + cache.rs (844)

Treat as shared refactor tranche in `assay-registry` after `verify` split:

- Extract parser/format/version handling from lockfile IO paths
- Isolate cache eviction and cache key semantics from transport/parsing
- Add determinism tests for cache key and lockfile normalization output

---

### 4.8 monitor.rs (833)

Split command orchestration from IO/rendering in `assay-cli`:

- `monitor/mod.rs` entrypoint + arg bridge
- `monitor/run.rs` execution loop
- `monitor/render.rs` TUI/text rendering
- `monitor/events.rs` event parsing and mapping
...
- [ ] **Semver/API check** (for library crates): `cargo semver-checks ...` passes
- [ ] **Feature matrix checks**: no-default + all-features (+ powerset where feasible)
- [ ] **No TODO placeholders**: no `todo!()`/`unimplemented!()` in final diff
- [ ] **Parser/crypto fuzz-property gates** present where applicable
- [ ] **Concurrency/model checks** present for runtime/store splits
- [ ] `cargo test -p <crate>` passes
- [ ] `cargo clippy -p <crate> --all-targets -- -D warnings -D clippy::todo -D clippy::unimplemented` passes
...
1. Create directory + `mod.rs` façade with final signatures and compilable wiring
2. Move blocks 1:1 (cut/paste); compile after each module
3. Fix imports (only after moves)
4. Add forbidden grep gates locally (then codify in CI)
5. Run `cargo test -p <crate>` + `cargo clippy -p <crate> --all-targets -- -D warnings -D clippy::todo -D clippy::unimplemented`
6. Run feature/API gates (`--no-default-features`, `--all-features`, semver checks)
7. Add or update behavior freeze tests + property tests
8. Ensure zero `todo!()` / `unimplemented!()` / placeholder panics in final PR
...
- [Rust 2024 Edition Guide](https://doc.rust-lang.org/nightly/edition-guide/rust-2024/index.html)
- [Cargo features reference](https://doc.rust-lang.org/cargo/reference/features.html)
- [RFC 8785 (JCS)](https://www.rfc-editor.org/rfc/rfc8785)
- [cargo-semver-checks](https://github.com/obi1kenobi/cargo-semver-checks)
- [cargo-fuzz](https://rust-fuzz.github.io/book/cargo-fuzz.html)
- [loom](https://github.com/tokio-rs/loom)
- [IACR ePrint 2025/980](https://eprint.iacr.org/2025/980)

---

## Appendix A: canonicalize move map (historical; already completed)
