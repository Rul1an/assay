...
3. Prefer the attested delegated proof pack emitted by the delegated workflow:
   ...
   What this catches and what it does not: a proof pack that was presented and
   did not hold up is no longer credited. An author who never presents one still
   reaches the fallback, because the upload step uses `if-no-files-found: warn`
   and the delegated workflow runs the definition on the dispatched ref. That
   case is bound by the lane classification requiring a delegated gate at all,
   and by review of the diff to the upload step.
...
## Required Lane Context (issue #1869, migration complete)

Branch protection on `main` requires the commit status `lane-check/proof`,
pinned to the GitHub Actions app. The status is posted by the lane helper on
the exact PR head, so a delegated same-head proof refreshes the required
context without a manual rerun. The `lane-check` Actions job still runs and
reports on every PR but is informational: a `workflow_run` execution cannot
refresh a check-run on the PR head, which is why the check-run was removed
from the required set (2026-07-27; empirical evidence on the tracking issue,
including the app-pin match and automatic stale-proof rejection within ~90 s
of a head move).
...
fresh delegated proof before the lane context reads clean again — measured at
one delegated cycle per advance (≈4m31s warm, ~13 min cold, the dispatch
being the only manual action). Batch main merges around open gated PRs
accordingly.
