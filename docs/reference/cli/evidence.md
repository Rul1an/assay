...
earlier one counts once (`audit_records_duplicate`). When several calls share one shape:
...
another value as given, or `unknown`). A call whose recorded decision is a denial and that still
