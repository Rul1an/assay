...
document, or one verified evidence bundle carrying `assay.sandbox.degraded`, into a
lossy observation.
...
| v1 | `active` | `applied` |
| v1 | `failed` | `degraded` |
| verified bundle | one `assay.sandbox.degraded` event | `degraded` |
...
Bundle input is integrity-verified before projection. The command selects by the
event type, validates the existing typed degradation payload, and refuses missing,
duplicate, unknown, tampered, oversized, or contradictory active-health input with
empty stdout. Its bundle limits are 64 KiB compressed input, 1 MiB decoded input,
512 KiB event data, 1,000 events, and 64 KiB per event line.
...
For standalone health JSON, the projection checks coherence of an already-typed
document, not authenticity or provenance. A coherent handwritten carrier can
therefore project. For bundle input, verification establishes bundle integrity;
it does not establish producer authenticity or enforcement efficacy.
