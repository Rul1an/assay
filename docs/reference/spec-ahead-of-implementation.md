...
| `assay_monitor_sendto` / `assay_monitor_sendmsg` | `assay-ebpf` ELF; `assay-monitor/src/probes.rs` | Compiled into the release program set and inventoried `Unsupported` with `AttachSpec::None`. Datagram coverage labels that depend on those tracepoints are unreachable on a live `assay monitor` run. Kernel `no_peer` / `non_ip` send counters also have no userspace readback | not closed: programs unattached, and the four userspace readbacks are absent |
