...
| `assay_monitor_sendto` / `assay_monitor_sendmsg` | `assay-ebpf` ELF; `assay-monitor/src/probes.rs` | Compiled into the release program set and inventoried `Unsupported` with `AttachSpec::None`. Runner archives (`assay runner-spike --kernel-capture` / `assay-runner-core` `network_protocol_coverage_for`) are count-derived, so datagram labels need send-event counts and are unreachable while those TPs stay unattached; CLI `assay monitor` `observation_health` is a different producer and does not emit datagram labels. Kernel `no_peer` / `non_ip` send counters also have no userspace readback | not closed: programs unattached, and the four userspace readbacks are absent |
