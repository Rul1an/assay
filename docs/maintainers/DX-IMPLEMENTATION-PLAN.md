...
| 5 | **reason_code everywhere** | reason_code in: console, job summary, summary.json; `reason_code_version: 1` in summary.json | ⚠️ reason_code ✅; reason_code_version niet in Summary (code) |
| 6 | **summary.json stable** | `schema_version` + `reason_code_version` in output; golden test | ⚠️ schema_version + reason_code ✅; reason_code_version ontbreekt |
...
**Definition of "default gate ready":** Alle P0-criteria groen; items 5–6 zijn nog ⚠️ (reason_code_version ontbreekt in code).
...
### Epic status (codebase verified 2026-01)

| Epic | Story | Status | Opmerking |
|------|-------|--------|-----------|
| E1 | E1.1 Template v2 | ✅ | `assay init --ci` → assay-action@v2; contract test |
| E1 | E1.2 Blessed entrypoint | ✅ | Docs: init --ci blessed, init-ci alias |
| E1 | E1.3 DX demo repos | ❌ | Geen examples/dx-demo-node of dx-demo-python |
| E2 | E2.1 JUnit default | ✅ | Action default --output junit; run.md JUnit path |
| E2 | E2.2 SARIF location invariant | ✅ | Unit + contract: elk result ≥1 location |
| E2 | E2.3 SARIF limits | ⚠️ | Evidence lint: truncate + "N omitted"; assay-core test SARIF geen truncatie |
| E2 | E2.4 Fork PR | ✅ | action.yml conditioneel; docs; action-tests fork_pr_sarif_skip |
| E3 | E3.1 Reason code registry | ✅ | exit_codes.rs; E_TRACE_NOT_FOUND, E_JUDGE_UNAVAILABLE, etc. |
| E3 | E3.2 summary.json versioned | ⚠️ | schema_version + reason_code in code; reason_code_version niet in Summary struct |
| E3 | E3.3 Compat switch | ✅ | --exit-codes=v2\|v1; ASSAY_EXIT_CODES |
| E3 | E3.4 reason_code in outputs | ✅ | Console, job summary, summary.json, SARIF waar van toepassing |
| E3 | E3.5 Docs + deprecation | ⚠️ | run.md exit table nog v1 (exit 3 = trace not found); reason_code/compat niet in run.md |
| E4 | E4.1 Next step in errors | ✅ | next_step in summary; suggest_next_steps in exit_codes |
| E4 | E4.2 Performance DX | ✅ | summary.json: slowest_tests, cache_hit_rate, phase_timings |
| E4 | E4.3 Progress N/M | ❌ | Alleen "Running N tests..." + eindsummary; geen N/M tijdens run |
...
- [x] `assay init --ci` → `.github/workflows/assay.yml` bevat `assay-action@v2` (golden/contract test).
- [x] Docs: init --ci = blessed; init-ci = alias; CI-integration + example repos link.
...
- [x] JUnit artifact + annotations bij failure met blessed snippet.
- [x] Unit: elk SARIF-result heeft `locations.length ≥ 1`; contract: schema 2.1.0 + upload-smoke.
- [ ] (P1) Truncatie + N omitted in run summary/SARIF description (evidence lint wel; assay ci SARIF niet).
- [x] (P1) Docs: fork = job summary only.
...
- [x] Missing trace → exit 2, reason_code E_TRACE_NOT_FOUND (v2); v1 legacy beschikbaar via --exit-codes=v1.
- [x] Judge unavailable (mock) → exit 3, reason_code E_JUDGE_UNAVAILABLE.
- [ ] summary.json bevat reason_code_version; reason_code in console, job summary, summary.json (en waar van toepassing SARIF) — reason_code ✅; reason_code_version ontbreekt in code.
- [ ] run.md en troubleshooting.md in lijn met gedrag; ADR-019 compatibility beschreven (run.md exit table nog v1; compat/reason_code niet gedocumenteerd).
...
- [x] Config/trace/test failure → stdout bevat minstens één suggestie (assay doctor / explain / baseline).
- [x] summary.json bevat slowest_tests (max 5), cache_hit_rate, phase_timings; console toont ze.
- [ ] Suite met 10+ tests → console toont progress (bijv. 3/10) — niet geïmplementeerd; alleen "Running N tests..." + eindsummary.
...
| **Status** | **✅ Sign-off (Step 3)** — Zie [docs/audit/E5-E8-VERIFICATION.md](audit/E5-E8-VERIFICATION.md), [E5-E8-TEST-REPORT.md](audit/E5-E8-TEST-REPORT.md). |
...
- [x] Golden tests: export (OTel, replay, SARIF, job summary) met default bevat geen prompt/response body.
...
- [x] DoD §8.3.5: semconv version in config/manifest; cardinality tests; redaction golden tests; config observability.md.
...
### OpenClaw harness-ready upgrades (E8/E5)

**E5/E8 Step 3 is afgetekend (audit-ready baseline);** zie [E5-E8-VERIFICATION.md](../audit/E5-E8-VERIFICATION.md). De sampling gate (`capture_requires_sampled_span` + `!span.is_disabled()`) is contract-tested; verdere upgrades hieronder zijn optioneel voor harness-productisatie.

Om E8/E5 verder "OpenClaw harness-ready" te maken, deze upgrades (na P1 SOTA):

| Upgrade | Doel | Detail |
|---------|------|--------|
| **Sampling / capture correctness** | Content-recording niet afhankelijk van tracing-internals. | Step 3: `!span.is_disabled()` + `capture_requires_sampled_span` is geïmplementeerd en contract-tested (geen blob/redact bij disabled span). Optioneel: exporter-layer hook of expliciete `should_capture_payload_now()` voor multi-subscriber omgevingen. |
| **BlobRef BYOS** | `assay.blob.ref` verwijst naar immutably stored payload (WORM-friendly) voor incident response. | Minimale metadata: `assay.blob.alg`, `assay.blob.scope`, `assay.blob.retention_class`. Sluit aan op OpenClaw incident-response (contain/rotate/audit). |
| **Gateway exposure guardrails** | Templates spiegelen OpenClaw exposure-advies (bind/port/firewall). | Collector standaard bind op **127.0.0.1** (niet 0.0.0.0); mTLS optie (enterprise); expliciet "tailnet only" profile (bv. Tailscale). |
| **Skill supply chain pack** | ClawHub is officieel publiek; pack = eerste "harness" entry. | Zie ROADMAP: openclaw-supplychain-baseline pack (OSS) + optionele CLI wrapper. |
...
**Status:** **✅ Step 3 sign-off** — Zie [E5-E8-VERIFICATION.md](../audit/E5-E8-VERIFICATION.md), [E5-E8-TEST-REPORT.md](audit/E5-E8-TEST-REPORT.md).
...
| 13 | E5 | **Privacy:** do-not-store-prompts default, redaction tests. **(Step 3 ✅ sign-off)** |
...
#### 8.3.1 Bleeding Edge: Stability & Version Gating

**Critical:** GenAI semconv evolves rapidly. We must treat it as a "moving target" with explicit stability gates.

```yaml
# Config
otel:
  genai_semconv_version: "1.28.0"      # Pinned version
  semconv_stability: "stable_only"     # or "experimental_opt_in"
  capture_mode: "off"                  # off | redacted_inline | blob_ref
```

| File | Change |
|------|--------|
| `crates/assay-core/src/otel/genai.rs` | `GenAiSemConv` trait abstraction + v1.28.0 impl |
| `crates/assay-core/src/config/otel.rs` | New `OtelConfig` struct with stability gates |

#### 8.3.2 Privacy & Scrubbing Strategy

**Hard Invariant:** No inline payloads by default. If capture is enabled, prefer "BlobRef" pattern.

| Mode | Behavior |
|------|----------|
| `off` (default) | No prompts/responses in span attributes |
| `blob_ref` | Payload stored in BYOS/blob store; span gets `assay.blob.ref` (Recommended) |
| `redacted_inline` | Legacy/Debug only. RegEx/structured scrubbing applied inline. |
...
**Metric Registry Rule:** `MetricRegistry` must panic/warn on registration of dynamic labels.

#### 8.3.4 Security: Telemetry Surface Guardrails ("OpenClaw" Defense)

If payload capture is enabled (`capture_mode != off`):
1. **Require TLS:** Reject `http://` OTLP endpoints.
2. **Explicit Allowlist:** `exporter.allowlist` must contain destination domain.
3. **No 0.0.0.0:** Rejects binding debug servers to 0.0.0.0.

#### 8.3.5 Definition of Done

- [x] **Semconv Abstraction:** Trait-based `GenAiSemConv` with v1.28.0 impl
- [x] **Stability Config:** `semconv_stability` gate implemented
- [x] **Privacy Modes:** `off` / `blob_ref` / `redacted_inline` logic
- [x] **Guardrails:** Boot check for TLS/Allowlist if capture is ON (Step 3 sign-off; zie [E5-E8-VERIFICATION.md](../audit/E5-E8-VERIFICATION.md))
- [ ] **Golden Tests:** Normalized snapshot verification (ignoring timestamps/IDs)
...
- [E5-E8-VERIFICATION](../audit/E5-E8-VERIFICATION.md) — E5/E8 Step 3 sign-off (observability & privacy baseline)
- [E5-E8-TEST-REPORT](../audit/E5-E8-TEST-REPORT.md) — test outcomes en sign-off bundle
...
- [ROADMAP](../project/ROADMAP.md) — strategic roadmap
