...
> **Sequence:** `T1a → T1b → G3 → P2a → H1 → P2b` shipped on `main` (**`a2a-signal-followup`** in **v3.3.0**). **`G4-A` Phase 1** (A2A `payload.discovery` evidence seam) is **shipped on `main`** — [G4-A freeze](architecture/G4-A-PHASE1-FREEZE.md), [PLAN-G4](architecture/PLAN-G4-A2A-DISCOVERY-CARD-EVIDENCE-2026q2.md). **`P2c`** — built-in **`a2a-discovery-card-followup`** (A2A-DC-001 / A2A-DC-002) — is **shipped on `main`** — [PLAN-P2c](architecture/PLAN-P2c-A2A-DISCOVERY-CARD-FOLLOWUP-PACK.md), [RFC-005](architecture/RFC-005-trust-compiler-mvp-2026q2.md) §6, [PLAN-G4 §P2c](architecture/PLAN-G4-A2A-DISCOVERY-CARD-EVIDENCE-2026q2.md#p2c--follow-on-not-g4). **G4-A** follow-up remains post-merge verification / release-truth hygiene only (no new G4 semantics). **`K1`** is now the active evidence-first line, and **`K1-A` Phase 1** is already **shipped on `main`** as the first bounded A2A `payload.handoff` seam; it remains adapter-first, evidence-first, and explicitly **not** a pack wave. **MCP authorization-discovery** remains the second candidate — see [PLAN-K1](architecture/PLAN-K1-A2A-HANDOFF-DELEGATION-ROUTE-EVIDENCE-2026q2.md), [K1-A freeze](architecture/K1-A-PHASE1-FREEZE.md), and [DISCOVERY-NEXT-EVIDENCE-WAVE-2026Q2.md](architecture/DISCOVERY-NEXT-EVIDENCE-WAVE-2026Q2.md). References: [PLAN-T1b](architecture/PLAN-T1b-TRUST-CARD-2026q2.md), [PLAN-G3](architecture/PLAN-G3-AUTHORIZATION-CONTEXT-EVIDENCE-2026q2.md), [PLAN-H1](architecture/PLAN-H1-TRUST-KERNEL-ALIGNMENT-RELEASE-HARDENING.md), [MIGRATION-TRUST-COMPILER-3.2.md](architecture/MIGRATION-TRUST-COMPILER-3.2.md).
...
March 2026 evidence and signal waves change the ordering inside Q3. `T1a`, `T1b`, `G3`, **`P2a`**, **`H1`**, **`P2b`** (`a2a-signal-followup`), **`G4-A` Phase 1** (A2A `payload.discovery`), and **`P2c`** (`a2a-discovery-card-followup`) are shipped on `main`. **G4-A** track: post-merge verification / release-truth hygiene only. **`K1`** is now the active evidence-first line, and **`K1-A` Phase 1** is shipped on `main` as the first bounded A2A `payload.handoff` seam; it remains adapter-first and is **not** a new pack slice — see [PLAN-K1](architecture/PLAN-K1-A2A-HANDOFF-DELEGATION-ROUTE-EVIDENCE-2026q2.md) and [K1-A freeze](architecture/K1-A-PHASE1-FREEZE.md).
...
| **P1** | `K1` A2A handoff / delegation-route evidence | Started on `main` via `K1-A` Phase 1: bounded route / handoff seam before any further pack slice. | Adapter-first, visibility-only, no pack in the same wave; see [PLAN-K1](architecture/PLAN-K1-A2A-HANDOFF-DELEGATION-ROUTE-EVIDENCE-2026q2.md) and [K1-A freeze](architecture/K1-A-PHASE1-FREEZE.md). |
| **P1** | `P2` Protocol Claim Packs (further slices) | ✅ **`P2b`** `a2a-signal-followup` (A2A-001..003) on **`v3.3.0`**. ✅ **`G4-A` Phase 1** on `main` (`payload.discovery`). ✅ **`P2c`** `a2a-discovery-card-followup` (A2A-DC-001..002) on `main` (`requires >=3.3.0`). **Next:** only after new first-class evidence is established. | [PLAN-P2b](architecture/PLAN-P2b-A2A-SIGNAL-FOLLOWUP-CLAIM-PACK.md); [PLAN-P2c](architecture/PLAN-P2c-A2A-DISCOVERY-CARD-FOLLOWUP-PACK.md); [PLAN-G4](architecture/PLAN-G4-A2A-DISCOVERY-CARD-EVIDENCE-2026q2.md); [G4-A freeze](architecture/G4-A-PHASE1-FREEZE.md). |
...
- [x] **`K1-A` Phase 1 — A2A handoff / delegation-route evidence** — first bounded `payload.handoff` seam shipped on `main`; route / handoff visibility only; no pack in the same wave — [PLAN-K1](architecture/PLAN-K1-A2A-HANDOFF-DELEGATION-ROUTE-EVIDENCE-2026q2.md), [K1-A freeze](architecture/K1-A-PHASE1-FREEZE.md)
- [ ] **Further `K1` widening only if separately justified** — any second adapter slice, MCP authorization-discovery move, or downstream pack remains a new maintainer decision, not an automatic follow-on
