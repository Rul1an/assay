...
> **Sequence:** `T1a → T1b → G3 → P2a → H1 → P2b` shipped on `main` (**`a2a-signal-followup`** first publicly released in **`v3.3.0`**). **`G4-A` Phase 1** (A2A `payload.discovery` evidence seam), **`P2c`** — built-in **`a2a-discovery-card-followup`** (A2A-DC-001 / A2A-DC-002), and **`K1-A` Phase 1** (`payload.handoff`) are now part of the public **`v3.4.0`** line. **G4-A** follow-up remains post-merge verification / release-truth hygiene only (no new G4 semantics). **`K1`** remains the active evidence-first line, adapter-first and explicitly **not** a pack wave. **MCP authorization-discovery** remains the second candidate — see [PLAN-K1](architecture/PLAN-K1-A2A-HANDOFF-DELEGATION-ROUTE-EVIDENCE-2026q2.md), [K1-A freeze](architecture/K1-A-PHASE1-FREEZE.md), and [DISCOVERY-NEXT-EVIDENCE-WAVE-2026Q2.md](architecture/DISCOVERY-NEXT-EVIDENCE-WAVE-2026Q2.md). References: [PLAN-T1b](architecture/PLAN-T1b-TRUST-CARD-2026q2.md), [PLAN-G3](architecture/PLAN-G3-AUTHORIZATION-CONTEXT-EVIDENCE-2026q2.md), [PLAN-H1](architecture/PLAN-H1-TRUST-KERNEL-ALIGNMENT-RELEASE-HARDENING.md), [MIGRATION-TRUST-COMPILER-3.2.md](architecture/MIGRATION-TRUST-COMPILER-3.2.md).
...
March 2026 evidence and signal waves change the ordering inside Q3. `T1a`, `T1b`, `G3`, **`P2a`**, **`H1`**, **`P2b`** (`a2a-signal-followup`), **`G4-A` Phase 1** (A2A `payload.discovery`), **`P2c`** (`a2a-discovery-card-followup`), and **`K1-A` Phase 1** (`payload.handoff`) are now public in **`v3.4.0`**. **G4-A** track: post-merge verification / release-truth hygiene only. **`K1`** remains the active evidence-first line; any widening beyond `K1-A` Phase 1 still needs a new bounded decision — see [PLAN-K1](architecture/PLAN-K1-A2A-HANDOFF-DELEGATION-ROUTE-EVIDENCE-2026q2.md) and [K1-A freeze](architecture/K1-A-PHASE1-FREEZE.md).
