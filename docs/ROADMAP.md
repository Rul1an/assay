...
> **Evidence portability (P31-P39):** Prepared for **`v3.6.0`**: supported external eval outcomes can enter the trust-compiler path as bounded receipts, starting with Promptfoo assertion-component results. This is a downstream evidence-portability lane, not a Promptfoo integration, partnership, or eval-run truth claim — see [From Promptfoo JSONL to Evidence Receipts](notes/FROM-PROMPTFOO-JSONL-TO-EVIDENCE-RECEIPTS.md).
