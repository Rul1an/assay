...
> Split refactor program is closed loop through Wave7C Step3 on `main` (see [plan](architecture/PLAN-split-refactor-2026q1.md), [report](architecture/REPORT-split-refactor-2026q1.md)).
...
| **2** | `T1b` Trust Card MVP | ✅ Merged on `main`: deterministic Trust Card artifacts from verified bundles (`trustcard.json`, `trustcard.md`, and static `trustcard.html` via `assay trust-card generate --out-dir`), derived from Trust Basis only. | Evidence-classified output; signed/attestation later; no generic risk score. |
