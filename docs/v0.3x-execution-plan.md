# Verdict v0.3.x Implementatieplan
...
2.  **v0.3.1 Adoption Release** (Done)
    ...
    *   `PR-3.1.2` verdict-action golden path
...
| **Trace misses / prompt mismatch** | `verdict trace verify` + coverage checks | v0.3.2 |
...
| **"Waarom threshold 0.85?"** | `verdict calibrate` + percentiles + aanbevelingen | v0.3.3 |
...
*   **Artifacts:** `verdict-linux-x86_64.tar.gz`, `verdict-macos-x86_64.tar.gz`, `verdict-macos-aarch64.tar.gz`, `SHA256SUMS`.
...
### PR-3.2.1 Trace core: ingest + verify
*   **CLI:**
    *   `verdict trace ingest --input logs/*.jsonl --output dataset.jsonl`
    *   `verdict trace verify --trace dataset.jsonl --config eval.yaml`
*   **Implementation:** `crates/verdict-core/src/trace/{mod,schema,ingest,verify}.rs`.
*   **Schema:** `TraceEntryV1` (standardized structure).

### PR-3.2.2 Precompute: embeddings + judge
*   **CLI:**
    *   `verdict trace precompute-embeddings --trace dataset.jsonl --embedder openai`
    *   `verdict trace precompute-judge --trace dataset.jsonl --judge openai`
*   **Contract:** Writes to `meta.verdict.embeddings` and `meta.verdict.judge`. Runner reads these before making live calls.
...
1.  Release workflow.
2.  Verdict Action Golden Path.
3.  Examples Pack.

**P1 (Offline CI & Lifecycle)**
4.  `verdict trace` (ingest/verify).
