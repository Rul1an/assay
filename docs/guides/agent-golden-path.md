...
| 2. Preflight | `invocation cwd` | `assay doctor --format json` | Success `0`; invalid explicit config `2`. | Parses as `assay.doctor_report.v0`. A config failure remains JSON and carries the top-level `reason_code` and `next_step` alongside `config_error.code`. | An unreadable explicit config is exit `2` and names the failing file in a concrete JSON argv next step. An absent explicit config reports the same `E_CFG_PARSE` identity as a malformed one; that identity is not split here. |
