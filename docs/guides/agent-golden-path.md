...
This source tree declares Assay `6.6.2` (`v6.6.2`).
This journey is pinned to Assay `6.6.1` ([`v6.6.1`](https://github.com/Rul1an/assay/releases/tag/v6.6.1)).
Install the CLI from a verified channel, then require `assay version` to print `6.6.1` before using the table below. Behavior merged after that tag is `Unreleased` and is not part of this release claim.

Upgrade by installing a newer explicit release and re-running all nine steps.
Roll back by reinstalling `v6.6.1` from the GitHub release assets and re-running the same journey.
