...
This journey is pinned to Assay `6.2.0` ([`v6.2.0`](https://github.com/Rul1an/assay/releases/tag/v6.2.0)).
Install the CLI from a verified channel, then require `assay version` to print `6.2.0` before using the table below. Behavior merged after that tag is `Unreleased` and is not part of this release claim.

Upgrade by installing a newer explicit release and re-running all nine steps.
Roll back by reinstalling `v6.2.0` from the GitHub release assets and re-running the same journey.
