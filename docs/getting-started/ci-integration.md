...
steps:
  - script: cargo install assay-cli --version 6.3.0 --locked
...
jobs:
  assay:
    ...
    steps:
      ...
      - run:
          name: Install Assay
          command: cargo install assay-cli --version 6.3.0 --locked
      - run:
          name: Run Tests
...
pipeline {
    ...
    stages {
        stage('Install Assay') {
            steps {
                sh 'cargo install assay-cli --version 6.3.0 --locked'
            }
        }
...
For environments without a preinstalled Rust toolchain, download and verify the explicit `v6.3.0` release asset during a trusted setup stage, then cache that exact binary. Assay does not currently claim a verified public GHCR image.
