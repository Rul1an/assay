...
The current release is Assay `6.3.0` (`v6.3.0`). Install the CLI from one of the verified channels below.
...
Download the asset for [`v6.3.0`](https://github.com/Rul1an/assay/releases/tag/v6.3.0), verify its published checksum, and place the binary on `PATH`.

Windows x86-64 uses:

```text
assay-v6.3.0-x86_64-pc-windows-msvc.zip
```
...
The generated [agent golden path](../guides/agent-golden-path.md) additionally uses `assay version`, whose release-pinned output is `6.3.0`.

## Development build

Behavior merged after `v6.3.0` is `Unreleased` and is not part of the release claim above.
...
```yaml
- name: Install Assay
  run: cargo install assay-cli --version 6.3.0 --locked
```
