...
The current release is Assay `6.7.0` (`v6.7.0`). Install the CLI from one of the verified channels below.
...
```bash
cargo install assay-cli --version 6.7.0 --locked
```
...
Download the asset for [`v6.7.0`](https://github.com/Rul1an/assay/releases/tag/v6.7.0), verify its published checksum, and place the binary on `PATH`.
...
```text
assay-v6.7.0-x86_64-pc-windows-msvc.zip
```
...
The `v6.7.0` image index is `ghcr.io/rul1an/assay-mcp-server@sha256:23670fb635d6200ba18778ee9fb3981264f0627f86fd369abe32cd799b1c1aad` (tags such as `v6.7.0`, `6.7`, and `latest` are convenience aliases; the digest is the pinned reference).

Pull and run the multi-arch `assay-mcp-server` image by index digest:

```bash
docker run --rm ghcr.io/rul1an/assay-mcp-server@sha256:23670fb635d6200ba18778ee9fb3981264f0627f86fd369abe32cd799b1c1aad --version
```
...
```bash
gh attestation verify oci://ghcr.io/rul1an/assay-mcp-server@sha256:23670fb635d6200ba18778ee9fb3981264f0627f86fd369abe32cd799b1c1aad -R Rul1an/assay --predicate-type https://slsa.dev/provenance/v1
gh attestation verify oci://ghcr.io/rul1an/assay-mcp-server@sha256:23670fb635d6200ba18778ee9fb3981264f0627f86fd369abe32cd799b1c1aad -R Rul1an/assay --predicate-type https://cyclonedx.org/bom
```

Verified status means [release run 36182068016](https://github.com/Rul1an/assay/actions/runs/36182068016) pulled the image by digest, verified attestations, and executed `--version` on both architectures.
...
The generated [agent golden path](../guides/agent-golden-path.md) additionally uses `assay version`, whose release-pinned output is `6.7.0`.
...
Behavior merged after `v6.7.0` is `Unreleased` and is not part of the release claim above.
...
```yaml
- name: Install Assay
  run: cargo install assay-cli --version 6.7.0 --locked
```
