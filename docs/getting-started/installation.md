...
The current release is Assay `6.4.0` (`v6.4.0`). Install the CLI from one of the verified channels below.
...
Download the asset for [`v6.4.0`](https://github.com/Rul1an/assay/releases/tag/v6.4.0), verify its published checksum, and place the binary on `PATH`.

Windows x86-64 uses:

```text
assay-v6.4.0-x86_64-pc-windows-msvc.zip
```
...
The `v6.4.0` image index is `ghcr.io/rul1an/assay-mcp-server@sha256:991a451b685d8b58447d3c4f5ff9b919b34e5bf85c44f605de6826662bc978b8` (tags such as `v6.4.0`, `6.4`, and `latest` are convenience aliases; the digest is the pinned reference).

Pull and run the multi-arch `assay-mcp-server` image by index digest:

```bash
docker run --rm ghcr.io/rul1an/assay-mcp-server@sha256:991a451b685d8b58447d3c4f5ff9b919b34e5bf85c44f605de6826662bc978b8 --version
```
...
```bash
gh attestation verify oci://ghcr.io/rul1an/assay-mcp-server@sha256:991a451b685d8b58447d3c4f5ff9b919b34e5bf85c44f605de6826662bc978b8 -R Rul1an/assay --predicate-type https://slsa.dev/provenance/v1
gh attestation verify oci://ghcr.io/rul1an/assay-mcp-server@sha256:991a451b685d8b58447d3c4f5ff9b919b34e5bf85c44f605de6826662bc978b8 -R Rul1an/assay --predicate-type https://cyclonedx.org/bom
```

Verified status means [release run 35147120941](https://github.com/Rul1an/assay/actions/runs/35147120941) pulled the image by digest, verified attestations, and executed `--version` on both architectures.
...
The generated [agent golden path](../guides/agent-golden-path.md) additionally uses `assay version`, whose release-pinned output is `6.4.0`.

## Development build

Behavior merged after `v6.4.0` is `Unreleased` and is not part of the release claim above.
...
```yaml
- name: Install Assay
  run: cargo install assay-cli --version 6.4.0 --locked
```
