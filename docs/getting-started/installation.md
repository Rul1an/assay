...
The current release is Assay `6.3.1` (`v6.3.1`). Install the CLI from one of the verified channels below.
...
Download the asset for [`v6.3.1`](https://github.com/Rul1an/assay/releases/tag/v6.3.1), verify its published checksum, and place the binary on `PATH`.
...
The generated [agent golden path](../guides/agent-golden-path.md) additionally uses `assay version`, whose release-pinned output is `6.3.1`.

## Development build

Behavior merged after `v6.3.1` is `Unreleased` and is not part of the release claim above.
...
```yaml
- name: Install Assay
  run: cargo install assay-cli --version 6.3.1 --locked
```
