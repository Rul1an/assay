...
### OpenClaw Governance Gateway (Positioning)

Assay positioneert zich niet als alternatief voor OpenClaw, maar als **Governance Gateway** ervoor: een hardening laag die policy enforcement, privacy-by-default telemetry en verifieerbare audit evidence levert rond de bestaande OpenClaw choke points (Gateway HTTP endpoints en skill lifecycle). Dit sluit aan op OpenClaw's eigen threat model rond exposure, prompt injection, logs/transcripts en incident response.

**Scope:** Niet OpenClaw runtime herschrijven of prompt-injection "oplossen"; wél governance op choke points + bewijsvoering + policy packs.
...
## Strategy: OpenClaw Hardening Kit

**Haalbaarheid:** Technisch hoog haalbaar in twee smaken: (1) Observability/Privacy harness (laag invasief), (2) Governance harness (proxy vóór Gateway). OpenClaw biedt duidelijke HTTP choke points en een officiële skill registry (ClawHub); Assay levert governance op die punten zonder runtime te forken.

**Choke points en deliverables:**

| Choke point | Rol van Assay | Deliverable |
|-------------|----------------|-------------|
| **Gateway HTTP** | Governance Reverse Proxy vóór OpenClaw Gateway: intercept `/tools/invoke`, `/v1/chat/completions`, `/v1/responses`; enforce policy, evidence capture, privacy filtering (BlobRef/RedactedInline) | P2: `assay-openclaw-proxy` (Rust service of OTel Collector + WASM/OPA); out-of-the-box config: loopback bind, allowlist endpoints, TLS-only |
| **Skills supply chain** | Skill Intake Gate: on ClawHub install/update — fetch bundle, static analysis (regex/heuristics + signature), allowlist (trusted publishers / pinned digests), SARIF + evidence bundle | P1/P2: `assay evidence lint --pack openclaw-baseline`; optioneel `assay openclaw clawhub install --policy strict` |
| **Observability** | OTel GenAI Privacy Firewall: default capture OFF; capture ON → TLS-only + allowlist + ack; content never inline → BlobRef; hardened collector template | P1: **E8/E5 defaults ✅ (Step 3 sign-off);** collector templates + baseline pack "No Prompt Leakage" + "Low-card metrics" + "TLS/allowlist required" (Q2) |

**Verifieerbare context (primair bronnen):** OpenClaw security docs benadrukken prompt injection, plugins/extensions als risico, incident response, network exposure (bind/port/firewall), logs/transcripts redaction en "rotate secrets if you suspect compromise". ClawHub is gedocumenteerd als publieke skill registry (public, open, versioned bundles + moderation/reporting). Gateway biedt WebSocket control plane én HTTP op dezelfde host/poort (o.a. `/v1/chat/completions`, `/v1/responses`, `/tools/invoke`). Release-versies en stars/forks: als "latest at time of writing" vermelden en naar releasepagina verwijzen i.p.v. hard te pinnen. **CVE/incidenten:** technische keten alleen als feit presenteren als primair geverifieerd; operationele implicatie wél hard zetten (patchen, token rotatie, exposure reduceren).
...
## Q2 2026: Supply Chain Security & OpenClaw Telemetry

**Objective:** Launch compliance and signing features with zero infrastructure cost; ship OpenClaw Hardening Kit (Telemetry Edition).
...
### OpenClaw Hardening Kit: Telemetry Edition (P1)

- [x] **E8/E5 defaults (Step 3 sign-off):** privacy-by-default, BlobRef (HMAC + ASSAY_ORG_SECRET required), RedactedInline, sampling gate, TLS/allowlist/localhost guardrails, VCR default_secure, SARIF/summary no prompt leakage. Zie [E5-E8-VERIFICATION](audit/E5-E8-VERIFICATION.md), [E5-E8-TEST-REPORT](audit/E5-E8-TEST-REPORT.md).
- [ ] **Collector templates** + **assay-openclaw docs**: secure-by-default deployment profiles (bind 127.0.0.1, TLS-only, allowlist)
- [ ] **Baseline pack**: "No Prompt Leakage" + "Low-card metrics" + "TLS/allowlist required"
...
| **P2** | GitHub Action v2.1 | Low | Medium | ✅ Complete |
| **P1** | E5/E8 Observability & privacy (Step 3) | Low | High | ✅ Sign-off (audit-ready) |
| **P2** | Telemetry Surface Guardrails (collector templates, packs) | Low | High | **Q2/Q3** |
...
**Objective:** Integration with the broader security ecosystem + agentic protocol support; OpenClaw Supply Chain & Governance Proxy.

### OpenClaw Hardening Kit: Supply Chain (P1)

- [ ] **openclaw-supplychain-baseline pack** (OSS): ClawHub skill linting — rules: "skill contains exec", "downloads remote binary", "writes outside workspace", "obfuscated commands", etc.
- [ ] **assay evidence lint --pack openclaw-baseline** (SARIF + evidence bundle)
- [ ] **Optioneel:** CLI wrapper voor install/update met policy enforcement (`assay openclaw clawhub install --policy strict`)
- [ ] **Future:** openclaw-supplychain-pro pack (Enterprise): org policies, allowlists, signing, approval workflow

### OpenClaw Governance Proxy (P2, Q3/Q4)

- [ ] **Reverse proxy** vóór Gateway endpoints: `/tools/invoke`, `/v1/chat/completions`, `/v1/responses`
- [ ] **Evidence export** per tool call + decision logs (CloudEvents + tracecontext)
- [ ] **Enterprise add-on:** approvals, RBAC, retention
...
**Planning:** For how we track this roadmap in GitHub (open-core only; enterprise stays off public GitHub), see [GITHUB-PROJECTS-USAGE.md](GITHUB-PROJECTS-USAGE.md) in this folder.

**See the roadmap / backlog:** [Assay Open-Core Project](https://github.com/users/Rul1an/projects/4).
