...
| 4 | ✅ **CI baseline-vergelijking + regressie-policy** | **Gedaan:** perf_main.yml (baseline) + perf_pr.yml (PR compare); Bencher reports met `sw/50x400b`, `sw/12xlarge`, `sr/wc`; thresholds (percentage test, upper_boundary 0.35 = 35%); perf_pr = warn. **Note:** GitHub Actions runners have ~20-30% variance; 25% threshold was too tight (same commit would pass/fail). **Later:** perf_pr_gate.yml met --err voor hard fail; thresholds per benchmark in Bencher UI. |
