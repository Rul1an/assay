...
CONFIG_FILE="$PWD/assay/python/examples/openai-demo/verdict.yaml" # Use default config but ensure ID matches if we use generic
...
LOOP_CONFIG="$PWD/assay/python/examples/openai-demo/verdict_loop_idemp.yaml"
cat > "$LOOP_CONFIG" <<EOF
version: 1
