...
pub async fn cmd_trace(args: TraceArgs) -> anyhow::Result<i32> {
    match args.cmd {
        ...
        TraceSub::PrecomputeEmbeddings {
            ...
            use std::sync::Arc;
            use verdict_core::providers::embedder::{
        ...
        TraceSub::PrecomputeJudge {
            ...
            // Reusing `verdict_core::judge::JudgeService::new` requires `JudgeStore` and `LlmClient`.
            ...
            store.init_schema()?;
            let judge_store = verdict_core::storage::judge_cache::JudgeCache::new(store);
            ...
            use std::sync::Arc;
            use verdict_core::providers::llm::{fake::FakeClient, openai::OpenAIClient, LlmClient};
            ...
            let config = verdict_core::judge::JudgeRuntimeConfig {
                enabled: true,
            ...
            let service = verdict_core::judge::JudgeService::new(config, judge_store, client);
