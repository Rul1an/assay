...
use verdict_core::validate::{validate, ValidateOptions, ValidateReport};
use verdict_core::errors::diagnostic::Diagnostic;
...
pub async fn run(args: ValidateArgs) -> anyhow::Result<i32> {
    ...
    let cfg = match load_config(&args.config) {
        ...
        Err(e) => {
             ...
             let diag = Diagnostic::new(verdict_core::errors::diagnostic::codes::E_CFG_PARSE, format!("Failed to load config: {}", e))
                .with_source("config")
                .with_context(json!({ "file": args.config }));

             print_report(&ValidateReport { diagnostics: vec![diag] }, &args.format);
             return Ok(2);
        }
    };
...
fn print_report(report: &ValidateReport, format: &str) {
    if format == "json" {
        let errors: Vec<&Diagnostic> = report.diagnostics.iter().filter(|d| d.severity == "error").collect();
        let warnings: Vec<&Diagnostic> = report.diagnostics.iter().filter(|d| d.severity == "warn").collect();
        ...
        // Text format
        let errors_count = report.diagnostics.iter().filter(|d| d.severity == "error").count();
        let warnings_count = report.diagnostics.iter().filter(|d| d.severity == "warn").count();

        if errors_count > 0 {
             eprintln!("✖ Validation failed ({} error{}, {} warning{})",
                errors_count, if errors_count != 1 { "s" } else { "" },
                warnings_count, if warnings_count != 1 { "s" } else { "" });
        } else if warnings_count > 0 {
             eprintln!("⚠️  Validation passed with warnings ({} warning{})",
                warnings_count, if warnings_count != 1 { "s" } else { "" });
        } else {
