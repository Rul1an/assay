...
pub async fn cmd_calibrate(args: CalibrateArgs) -> anyhow::Result<i32> {
    let report = if let Some(run_path) = &args.run {
        ...
        let run: verdict_core::report::RunArtifacts = serde_json::from_str(&raw)?;
        verdict_core::calibration::from_run(&run, args.target_tail)?
    } else {
        ...
        verdict_core::calibration::from_db(&store, &suite, args.last, args.target_tail)?
    };
