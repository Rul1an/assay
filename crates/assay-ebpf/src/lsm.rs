...
fn emit_event(ctx: &LsmContext, event_id: u32, _cgroup_id: u64, _rule_id: u32, data: &[u8], _path_len: u32) {
    ...
    // DEBUG: Attempting Reserve
    // info!(ctx, "🐛 DEBUG: Reserving for Event {}", event_id);
    ...
    if let Some(mut entry) = LSM_EVENTS.reserve::<[u8; 520]>(0) {
        ...
        entry.submit(0);
        // info!(ctx, "🐛 DEBUG: Event {} Submitted", event_id);
    } else {
        info!(ctx, "❌ ERROR: RingBuf Reserve Failed for Event {}", event_id);
    }
}
...
fn try_file_open_lsm(ctx: LsmContext) -> Result<i32, i64> {
    ...
    // info!(&ctx, "🐛 DEBUG: Emitting Event 108");
    emit_event(&ctx, 108, cgroup_id, 0, &dbg_entry, 0);
    ...
    // info!(&ctx, "🐛 DEBUG: Emitting Event 112 dev={} ino={}", s_dev, i_ino);
