...
pub async fn precompute_embeddings(
    ...
    for (i, line) in reader.lines().enumerate() {
        ...
        let has_embedding = entry.meta.pointer("/verdict/embeddings/response").is_some()
            && entry
                .meta
        ...
        if !has_embedding {
            ...
            // BUT, the runner logic expects `meta.verdict.embeddings.reference`.
            ...
            if let Some(tc) = matching_tc {
                ...
                } = &tc.expected
                {
                    ...
                    if !entry.meta.get("verdict").is_some_and(|v| v.is_object()) {
                        entry.meta["verdict"] = serde_json::json!({});
                    }

                    entry.meta["verdict"]["embeddings"] = serde_json::json!({
                        "model": model,
                        "response": resp_vec,
                        "reference": ref_vec,
        ...
        if let Some(tc) = matching_tc {
            ...
            if rubric_id != "none" {
                ...
                if !existing {
                    ...
                    // and typically writes to `verdict.judge.{rubric_id}`.
                    judge
                        .evaluate(
                            &entry.request_id,
                            rubric_id,
