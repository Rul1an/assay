...
fn summarize_trace(path: &Path, _cfg: &EvalConfig, _diags: &mut Vec<Diagnostic>) -> anyhow::Result<TraceSummary> {
    ...
    let mut has_verdict_meta = false;
    ...
    for (i, line) in rdr.lines().enumerate() {
        ...
        // scan first 200 entries for verdict meta coverage
        if i < 200 {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&line) {
...
fn summarize_baseline(path: &Path, _diags: &mut Vec<Diagnostic>) -> anyhow::Result<BaselineSummary> {
    ...
    Ok(BaselineSummary {
        ...
        verdict_version: Some(b.verdict_version.clone()),
...
fn summarize_caches(notes: &mut Vec<String>) -> CacheSummary {
    // best effort: read HOME and check ~/.verdict/*
    let home = std::env::var("HOME").ok();
    if home.is_none() {
        notes.push("HOME not set; cannot inspect ~/.verdict caches".to_string());
        return CacheSummary::default();
    }
    let home = home.unwrap();
    let cache_dir = format!("{}/.verdict/cache", home);
    let emb_dir = format!("{}/.verdict/embeddings", home);

    CacheSummary {
        verdict_cache_dir: Some(cache_dir.clone()),
        verdict_embeddings_dir: Some(emb_dir.clone()),
...
fn suggest_from(
    ...
    if diags.iter().any(|d| d.code == codes::E_TRACE_MISS) {
        out.push(SuggestedAction {
            ...
            steps: vec![
                "Run: verdict trace verify --trace <trace.jsonl> --config <eval.yaml>".into(),
    ...
    if diags.iter().any(|d| d.code == codes::E_REPLAY_STRICT_MISSING) {
        out.push(SuggestedAction {
            ...
            steps: vec![
                "Run: verdict trace precompute-embeddings --trace <trace.jsonl> --output <trace_enriched.jsonl> ...".into(),
                "Run: verdict trace precompute-judge --trace <trace_enriched.jsonl> --output <trace_enriched.jsonl> ...".into(),
            ],
        });
    }
    ...
    if diags.iter().any(|d| d.code == codes::E_BASE_MISMATCH) {
        out.push(SuggestedAction {
            ...
            steps: vec![
                "Export on main: verdict ci --config <eval.yaml> --trace-file <main.jsonl> --export-baseline baseline.json".into(),
                "Gate PR: verdict ci --baseline baseline.json".into(),
            ],
        });
    }
        ...
        if t.entries > 50_000 {
            out.push(SuggestedAction {
                ...
                steps: vec![
                    ...
                    "Enable incremental: verdict ci --incremental".into(),
