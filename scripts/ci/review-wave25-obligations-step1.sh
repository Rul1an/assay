...
PLAN_FILE="docs/contributing/SPLIT-PLAN-wave25-obligations-log.md"
...
while IFS= read -r f; do
  ...
  if [[ "$f" == .github/workflows/* ]]; then
    echo "FAIL: Wave25 Step1 must not touch workflows ($f)"
    exit 1
  fi
  ...
  if [[ "$ok" != "true" ]]; then
    echo "FAIL: file not allowed in Wave25 Step1: $f"
    exit 1
  fi
...
for p in "${FROZEN_PATHS[@]}"; do
  if git diff --name-only "$BASE_REF"...HEAD -- "$p" | rg -n '.' >/dev/null; then
    echo "FAIL: Wave25 Step1 must not change frozen path: $p"
...
rg -n '^# SPLIT PLAN - Wave25 Obligations Log Execution$' "$PLAN_FILE" >/dev/null || {
  echo "FAIL: missing plan title"
  exit 1
}

for marker in "log" "legacy_warning" "obligation_outcomes"; do
  rg -n "\\b${marker}\\b" "$PLAN_FILE" >/dev/null || {
...
for status in "applied" "skipped" "error"; do
  rg -n "\\b${status}\\b" "$PLAN_FILE" >/dev/null || {
    echo "FAIL: missing obligation outcome status: $status"
    exit 1
  }
done

for nongoal in "approval_required" "restrict_scope" "redact_args"; do
  rg -n "$nongoal" "$PLAN_FILE" >/dev/null || {
    echo "FAIL: missing non-goal marker: $nongoal"
    exit 1
  }
done
