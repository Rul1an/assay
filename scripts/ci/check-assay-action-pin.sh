...
# consumer workflows in one check. Offline: no live GitHub fetch.
set -euo pipefail
...
python3 - "${PIN}" "${FIXTURE}" "${PROVENANCE}" "${WORKFLOW_ASSAY}" "${WORKFLOW_V2}" <<'PY'
...
def is_assay_action_ref(ref: str) -> bool:
    ...
    return lowered == "./assay-action" or lowered.startswith("./assay-action/") or "assay-action@" in lowered
...
def check_workflow(path: Path, allowed_inputs: set[str]) -> None:
    if not path.is_file():
        fail(f"in-scope workflow missing: {path}")
    ...
    if found == 0:
        fail(f"{path}: no Rul1an/assay-action@{PIN} uses found")
...
def main() -> None:
    ...
    for workflow in WORKFLOWS:
        check_workflow(workflow, allowed)
    print(
        f"assay action consumer pin: {PIN} "
        f"(fixture sha256 {digest}; {len(WORKFLOWS)} workflows)"
    )
