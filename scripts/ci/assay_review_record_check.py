...
def evaluate(live_sha: str, branch_ref: str, comments: list[dict[str, Any]]) -> None:
    ...
    for comment in comments:
        ...
        if record is None or str(record.get("head_sha") or "").lower() != live_sha.lower():
            continue
        if str((comment.get("user") or {}).get("type") or "") == "Bot":
            raise GateError("bot_carrier", "Bot")
        ...
        if str(comment.get("updated_at") or "") != str(comment.get("created_at") or ""):
...
class GitHubApi:
    ...
    def get(self, path: str) -> Any:
        ...
        try:
            ...
            kind = "comments_api_failure" if "comments" in path else "pr_api_failure"
...
def _cmt(record, *, extra="", second=False, bot=False, edited=False, login="Rul1an", body=None):
    ...
    return {
        ...
        "created_at": "t0", "updated_at": "t1" if edited else "t0",
    }
