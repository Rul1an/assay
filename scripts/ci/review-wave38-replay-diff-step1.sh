...
while IFS= read -r f; do
  ...
  if [[ "$f" == .github/workflows/* ]]; then
    echo "FAIL: Wave38 Step1 must not touch workflows ($f)"
    exit 1
  fi
  ...
  if [[ "$ok" != "true" ]]; then
    echo "FAIL: file not allowed in Wave38 Step1: $f"
    exit 1
  fi
...
for p in "${FROZEN_PATHS[@]}"; do
  if git diff --name-only "$BASE_REF"...HEAD -- "$p" | rg -n '.' >/dev/null; then
    echo "FAIL: Wave38 Step1 must not change frozen path: $p"
...
rg -n '^# SPLIT PLAN - Wave38 Replay Diff Contract$' \
  docs/contributing/SPLIT-PLAN-wave38-replay-diff-contract.md >/dev/null || {
...
for marker in \
  ...
  'evidence_only'
do
  rg -n "$marker" docs/contributing/SPLIT-PLAN-wave38-replay-diff-contract.md >/dev/null || {
