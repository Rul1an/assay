...
echo '== Agentic Step2 quality checks =='
cargo fmt --check
...
echo '== Agentic Step2 facade invariants =='
...
echo '== Agentic Step2 public surface invariants =='
...
echo '== Agentic Step2 builder visibility invariants =='
...
echo '== Agentic Step2 file presence invariants =='
test -f "${mod_file}"
test -f "${builder_file}"
test -f "${helpers_file}"
test -f "${tests_file}"

echo '== Agentic Step2 test-name invariants =='
...
echo '== Agentic Step2 diff allowlist =='
leaks="$({ git diff --name-only "${base_ref}...HEAD" | \
  "${rg_bin}" -v '^crates/assay-core/src/agentic/.*\.rs$|^docs/contributing/SPLIT-CHECKLIST-agentic-step2.md$|^docs/contributing/SPLIT-MOVE-MAP-agentic-step2.md$|^docs/contributing/SPLIT-REVIEW-PACK-agentic-step2.md$|^scripts/ci/review-agentic-step2.sh$' || true; })"
if [ -n "${leaks}" ]; then
...
if git diff --name-only "${base_ref}...HEAD" | "${rg_bin}" '^\.github/workflows/'; then
  echo 'workflow changes are forbidden in agentic Step2'
  exit 1
fi

echo 'Agentic Step2 reviewer script: PASS'
