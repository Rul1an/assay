...
ALLOWLIST=(
  ...
  "docs/contributing/SPLIT-CHECKLIST-wave31-redact-args-step2.md"
  "docs/contributing/SPLIT-MOVE-MAP-wave31-redact-args-step2.md"
  "docs/contributing/SPLIT-REVIEW-PACK-wave31-redact-args-step2.md"
  "scripts/ci/review-wave31-redact-args-step2.sh"
)
...
while IFS= read -r f; do
  ...
  if [[ "$f" == .github/workflows/* ]]; then
    echo "FAIL: Wave31 Step2 must not touch workflows ($f)"
    exit 1
  fi
  ...
  if [[ "$ok" != "true" ]]; then
    echo "FAIL: file not allowed in Wave31 Step2: $f"
    exit 1
  fi
done < <(git diff --name-only "$BASE_REF"...HEAD)

echo "[review] marker checks: redact_args shape + evidence"
for marker in \
  'redact_args' \
  'redaction_target' \
  'redaction_mode' \
...
echo "[review] no redact_args execution/enforcement in this wave"
if rg -n \
