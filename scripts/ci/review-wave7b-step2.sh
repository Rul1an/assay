...
if [ ! -d "${loader_root}" ]; then
  echo "Step2 precondition not met: missing ${loader_root}"
  exit 1
fi
if [ ! -d "${store_root}" ]; then
  echo "Step2 precondition not met: missing ${store_root}"
  exit 1
fi
...
do
  ...
  cargo test -p assay-evidence "${test_name}" -- --exact
done

echo "== Wave7B Step2 contract anchors (store) =="
...
echo "== Wave7B Step2 facade gates =="
...
check_no_match_code_only \
  'serde_yaml::from_str|serde_jcs::to_string|Sha256::digest|hex::encode' \
  "${loader_facade}"
...
check_no_match_code_only \
  'PRAGMA table_info\(|ALTER TABLE .* ADD COLUMN|SELECT id, episode_id, idx, kind, name, content' \
  "${store_facade}"

echo "== Wave7B Step2 single-source gates =="
check_only_file_matches \
...
echo "Wave7B Step2 reviewer script: PASS"
