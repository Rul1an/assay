#!/usr/bin/env bash
# Three-run determinism wrapper for the Gemini Python google-genai
# second-runtime fixture. Mirrors
# scripts/ci/runner-spike-openai-agents-kernel-policy-three-run-determinism.sh
# structurally: runs the acceptance script three times against the same
# cassette and verifies byte-identical normalized artifacts.
set -euo pipefail
...
if [ "$(uname -s)" != "Linux" ]; then
  echo "SKIP: runner-spike Gemini kernel+policy three-run determinism requires Linux." >&2
  exit 40
fi
...
if [ -n "${ASSAY_RUNNER_DELEGATED_PROOF_GATE_DIR:-}" ]; then
  ...
  TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/assay-runner-gemini-kernel-policy-determinism.XXXXXX")"
  cleanup() {
    rm -rf -- "$TMP_ROOT"
  }
fi
...
RUN_ID="${ASSAY_RUNNER_ACCEPTANCE_RUN_ID:-run_gemini_google_genai_kernel_policy_determinism}"

for run in 1 2 3; do
  # Reset the measured work-dir between runs so the kernel layer sees the
  # same create-vs-open-existing pattern in all three runs, matching the
  # S5 determinism wrapper.
  rm -rf -- "$WORK_DIR"
  echo "=== runner-spike Gemini google-genai kernel+policy determinism run $run/3 ==="
  ASSAY_RUNNER_ACCEPTANCE_ARTIFACT_DIR="$TMP_ROOT/run-$run" \
    ...
    "$ROOT/scripts/ci/runner-spike-gemini-google-genai-acceptance.sh"
done
...
for run in (2, 3):
    ...
    if sdk != baseline_sdk:
        fail_text_change(
            ...
            sdk,
            run,
            "; the deterministic Gemini google-genai fixture should emit byte-stable normalized events.",
        )
...
print("runner-spike Gemini google-genai kernel+policy three-run determinism verified")
PY

echo "PASS: runner-spike Gemini google-genai kernel+policy three-run determinism"
