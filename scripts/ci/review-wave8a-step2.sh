...
rg_bin="$(command -v rg)"
legacy_file="crates/assay-adapter-a2a/src/lib.rs"
new_root="crates/assay-adapter-a2a/src"
...
check_no_increase() {
  ...
  local before after
  before="$(
    git show "${base_ref}:${legacy_file}" | { "$rg_bin" -n "$pattern" || true; } | wc -l | tr -d ' '
  )"
  after="$(
    { "$rg_bin" -n "$pattern" "$new_root" -g'*.rs' || true; } | wc -l | tr -d ' '
  )"
...
echo "== Wave8A Step2 quality checks =="
cargo fmt --check
...
echo "== Wave8A Step2 contract anchors =="
for test_name in \
  adapter_impl::tests::protocol_metadata_uses_exact_version_and_range_capability \
  adapter_impl::tests::strict_task_requested_fixture_maps_expected_event \
  adapter_impl::tests::strict_missing_task_id_fails_with_measurement_error \
  adapter_impl::tests::lenient_missing_task_id_substitutes_unknown_task \
...
echo "== Wave8A Step2 facade gates =="
check_has_match '^mod adapter_impl;$' 'crates/assay-adapter-a2a/src/lib.rs'
check_no_match '^fn parse_packet\(|^fn validate_protocol\(|^fn observed_version\(|^fn build_payload\(|^fn normalize_json\(' 'crates/assay-adapter-a2a/src/lib.rs'

echo "== Wave8A Step2 single-source gates =="
...
check_only_file_matches 'fn observed_version\(|fn validate_supported_version\(|fn parse_version\(' "$new_root" 'adapter_impl/version.rs'
check_only_file_matches 'fn string_field\(|fn nested_string_field\(|fn nested_string_array_field\(|fn timestamp_field\(|fn default_time\(' "$new_root" 'adapter_impl/fields.rs'
...
check_only_file_matches 'fn build_payload\(|fn normalize_json\(' "$new_root" 'adapter_impl/payload.rs'
...
if git diff --name-only "${base_ref}...HEAD" | "$rg_bin" '^\.github/workflows/'; then
  echo "workflow changes are forbidden in Wave8A Step2"
  exit 1
fi

echo "Wave8A Step2 reviewer script: PASS"
