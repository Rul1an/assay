...
serde_file='crates/assay-core/src/model/serde.rs'
validation_file='crates/assay-core/src/model/validation.rs'
tests_file='crates/assay-core/src/model/tests/mod.rs'

echo '== Model Step3 quality checks =='
cargo fmt --check
...
echo '== Model Step3 closure invariants =='
if [ ! -f "${facade}" ]; then
...
check_no_io "${serde_file}"
check_no_io "${validation_file}"
...
test_count="$("${rg_bin}" -n '^\s*fn\s+test_' "${tests_file}" | wc -l | tr -d ' ')"
if [ "${test_count}" -lt 5 ]; then
  echo "expected at least 5 relocated tests in ${tests_file}, got ${test_count}"
  exit 1
fi

echo '== Model Step3 diff allowlist =='
if [[ "${base_ref}" == *"wave13-model-step2-mechanical"* ]]; then
  allowlist_pattern='^docs/contributing/SPLIT-CHECKLIST-model-step3.md$|^docs/contributing/SPLIT-REVIEW-PACK-model-step3.md$|^scripts/ci/review-model-step3.sh$'
else
  allowlist_pattern='^crates/assay-core/src/model\.rs$|^crates/assay-core/src/model/.*\.rs$|^docs/contributing/SPLIT-PLAN-wave13-model.md$|^docs/contributing/SPLIT-CHECKLIST-model-step1.md$|^docs/contributing/SPLIT-REVIEW-PACK-model-step1.md$|^scripts/ci/review-model-step1.sh$|^docs/contributing/SPLIT-CHECKLIST-model-step2.md$|^docs/contributing/SPLIT-MOVE-MAP-model-step2.md$|^docs/contributing/SPLIT-REVIEW-PACK-model-step2.md$|^scripts/ci/review-model-step2.sh$|^docs/contributing/SPLIT-CHECKLIST-model-step3.md$|^docs/contributing/SPLIT-REVIEW-PACK-model-step3.md$|^scripts/ci/review-model-step3.sh$'
fi
...
if git diff --name-only "${base_ref}...HEAD" | "${rg_bin}" '^\.github/workflows/'; then
  echo 'workflow changes are forbidden in model Step3'
  exit 1
fi

echo 'Model Step3 reviewer script: PASS'
