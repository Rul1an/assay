...
while IFS= read -r f; do
  ...
  if [[ "$f" == .github/workflows/* ]]; then
    echo "FAIL: Wave28 Step1 must not touch workflows ($f)"
    exit 1
  fi
  ...
  if [[ "$ok" != "true" ]]; then
    echo "FAIL: file not allowed in Wave28 Step1: $f"
    exit 1
  fi
...
for p in "${FROZEN_PATHS[@]}"; do
  if git diff --name-only "$BASE_REF"...HEAD -- "$p" | rg -n '.' >/dev/null; then
    echo "FAIL: Wave28 Step1 must not change frozen path: $p"
...
rg -n '^# SPLIT PLAN — Wave28 Approval Required Enforcement$' \
  docs/contributing/SPLIT-PLAN-wave28-approval-required.md >/dev/null || {
...
for marker in \
  ...
  'approval_required'
do
  rg -n "$marker" docs/contributing/SPLIT-PLAN-wave28-approval-required.md >/dev/null || {
