...
echo '== ACP Step2 quality checks =='
cargo fmt --check
cargo clippy -p assay-adapter-acp -p assay-adapter-api --all-targets -- -D warnings
cargo test -p assay-adapter-acp

echo '== ACP Step2 scope checks =='
leaks="$({ git diff --name-only "${base_ref}...HEAD" | \
  "${rg_bin}" -v '^crates/assay-adapter-acp/src/lib\.rs$|^crates/assay-adapter-acp/src/adapter_impl/|^crates/assay-adapter-acp/src/tests/mod\.rs$|^docs/contributing/SPLIT-CHECKLIST-acp-step2\.md$|^docs/contributing/SPLIT-MOVE-MAP-acp-step2\.md$|^docs/contributing/SPLIT-REVIEW-PACK-acp-step2\.md$|^scripts/ci/review-acp-step2\.sh$' || true; })"
if [ -n "${leaks}" ]; then
...
if git diff --name-only "${base_ref}...HEAD" | "${rg_bin}" '^\.github/workflows/'; then
  echo 'workflow changes are forbidden in ACP Step2'
  exit 1
fi

if git status --porcelain -- crates/assay-adapter-acp/src | "${rg_bin}" '^\?\?'; then
  echo 'untracked files under crates/assay-adapter-acp/src/** are forbidden in Step2'
  exit 1
fi

echo '== ACP Step2 facade invariants =='
...
echo '== ACP Step2 visibility invariants =='
...
echo '== ACP Step2 test relocation invariants =='
...
echo 'ACP Step2 reviewer script: PASS'
