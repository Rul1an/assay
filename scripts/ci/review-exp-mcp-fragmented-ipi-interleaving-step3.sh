...
while IFS= read -r f; do
  ...
  if [[ "$f" == .github/workflows/* ]]; then
    echo "FAIL: Wave23 Step3 must not touch workflows ($f)"
    exit 1
  fi
  ok="false"
  ...
  if [[ "$ok" != "true" ]]; then
    echo "FAIL: file not allowed in Wave23 Step3: $f"
    exit 1
  fi
done < <(git diff --name-only "$BASE_REF"...HEAD)

echo "[review] re-run Step2 markers (run-shape + interleaving fields)"
...
rg -n 'second_sink|interleaving' scripts/ci/exp-mcp-fragmented-ipi/mock_mcp_server.py >/dev/null || {
  echo "FAIL: interleaving mixed-sink marker missing in mock server"
  exit 1
}
rg -n 'segment_label|turn_index|case_id|blocked_before_attempt|success_any_sink_canary' \
  ...
  echo "FAIL: interleaving publication fields missing"
  exit 1
}
rg -n 'sink_attempted_rate|blocked_before_attempt_rate|protected_false_positive_rate_ci' \
  scripts/ci/exp-mcp-fragmented-ipi/score_interleaving.py >/dev/null || {
  echo "FAIL: scorer derived-rate/CI fields missing"
  exit 1
}
