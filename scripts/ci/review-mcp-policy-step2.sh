...
echo '== MCP Policy Step2 quality checks =='
cargo fmt --check
...
leaks="$({ git diff --name-only "${base_ref}...HEAD" | \
  "${rg_bin}" -v '^crates/assay-core/src/mcp/policy\.rs$|^crates/assay-core/src/mcp/policy/|^docs/contributing/SPLIT-CHECKLIST-mcp-policy-step2\.md$|^docs/contributing/SPLIT-MOVE-MAP-mcp-policy-step2\.md$|^docs/contributing/SPLIT-REVIEW-PACK-mcp-policy-step2\.md$|^scripts/ci/review-mcp-policy-step2\.sh$' || true; })"
if [ -n "${leaks}" ]; then
...
if git diff --name-only "${base_ref}...HEAD" | "${rg_bin}" '^\.github/workflows/'; then
  echo 'workflow changes are forbidden in MCP Policy Step2'
  exit 1
fi

if git status --porcelain -- crates/assay-core/src/mcp/policy | "${rg_bin}" '^\?\?'; then
  echo 'untracked files under crates/assay-core/src/mcp/policy/** are forbidden in Step2'
  exit 1
fi

echo '== MCP Policy Step2 facade invariants =='
...
echo '== MCP Policy Step2 visibility invariants =='
...
echo 'MCP Policy Step2 reviewer script: PASS'
