#!/usr/bin/env python3
"""Fail when published adequacy prose stops agreeing with its typed measurement.
...
deterministic projections of two bounded `.md.in` templates and the current typed
rows in `adequacy/results.json`. The committed Markdown must equal a fresh render.
...
3. Transcription and subject/tool provenance remain pinned to public prose.
4. Projection. Every committed output equals the renderer over current typed rows.
5. Sweep. No `N of M` or percentage may be authored in a template unless it is a
   generated token or a reasoned `not_derived` exemption in that template.

THE GAP, NAMED RATHER THAN IMPLIED. This makes generated scalars un-rottable; it
does not make the surrounding prose true. Three specific holes:
...
import project_published_numbers as projection
...
# Generated documents covered by this checker. Their paired templates and exemption metadata are
# fixed by projection.document_pairs(); discovery would let a document opt out by disappearing.
DOCUMENTS = [
...
def strip_block(text: str) -> str:
    """Everything outside generated-number metadata and editorial exemption reasons."""
...
def check() -> list[str]:
    ...
    try:
        findings.extend(projection.projection_findings(REPO))
    except (OSError, UnicodeDecodeError, ValueError) as exc:
        findings.append("published-number projection failed: %s" % exc)

    # ---- 5. unbound number-shaped template prose --------------------------
    for template, _output in projection.document_pairs(REPO):
        rel = template.relative_to(REPO)
        try:
            _source, metadata, body = projection.template_parts(template)
        except (OSError, UnicodeDecodeError, ValueError) as exc:
            findings.append(str(exc))
            continue
        exemptions = metadata.get("not_derived")
        if not isinstance(exemptions, list):
            findings.append("%s: checked-numbers metadata needs a not_derived list" % rel)
            continue
        swept = normalise(projection.TOKEN.sub(" ", body))
        for entry in exemptions:
            if not isinstance(entry, dict):
                findings.append("%s: not_derived entries must be objects" % rel)
                continue
            token, reason = entry.get("token"), entry.get("reason")
            if not isinstance(token, str) or not token or not str(reason or "").strip():
        ...
        for leftover in SWEEP.finditer(swept):
            ...
            findings.append("%s: %r is authored in the template but is neither a generated token "
                            "nor declared not_derived. Context: ...%s..."
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print("published adequacy projection is not current:\n")
        for f in findings:
            print("  - %s\n" % f)
    else:
        print("published adequacy projection agrees with the current typed rows")
