...
python3 conformance/run_all.py --require-complete --completion-scope required --with-cargo
...
| [`privileged-mcp-action-producer`](privileged-mcp-action-v0/) | one-decision write + two-decision still importable | `cargo test -p assay-cli --bin assay producer_lane_` | producer writes record shape; does not call the verifier |
| [`privileged-mcp-action-verifier`](privileged-mcp-action-v0/) | 14 (5 accept, 9 reject) | `cargo test -p assay-cli --test conformance_privileged_mcp_action conformance_corpus_reproduces_all_expected_outcomes` | distinct id from `privileged-mcp-action-v0` ([#1840](https://github.com/Rul1an/assay/issues/1840)) |
...
| [`privileged-mcp-action-e2e`](privileged-mcp-action-v0/) | 1 golden path | `cargo test -p assay-cli --bin assay e2e_lane_` | one produced bundle then verifier; not corpus 14/14 |
...
|---|---|---|
| `mcp-jsonrpc-id-conformance` | module | **6 of 10 in scope (60.0%), 4 survivors, 7 out of scope.** The first score was **4 of 4** over the presence/null arms alone. The positive control is a string id and RequestId is string or number, so the type arms belong in the denominator: string is isolated, number and bool are not. The envelope rules outside that denominator were re-checked against the three published messages; none of them moves an outcome, and the reasons still hold |
| [observed-effect-drift-consumer](https://github.com/Rul1an/observed-effect-v0) @ `dc10fad0c` | batch | **14 of 23 in scope (60.9%), 9 survivors.** The first score was **4 of 5** over merge-policy rules only. The 14 case names announce the recompute, advisory, and profile rules as well. The original survivor remains (the body can be read whether or not the ref recomputed); the new ones are the fail-closed recompute siblings the cases never present, the RFC 8785 UTF-16 key-order rule the consumer claims but no body isolates, and a redacted-field conjunct that `incomplete_missing_non_claims` does not distinguish from a missing field |
| `privileged-mcp-action-v0` | process | **6 of 28 in scope (21.4%), 22 survivors, 4 out of scope, 2 known holes.** Declared three times before it said anything true. First three rules and *no result*; then eight read off the verifier path, scoring 50%, published with a pattern claim. **The denominator was the finding, not the score.** An adversarial re-measurement deleted a further set of promised rules in one build and reproduced every outcome and every claim matrix, so the declaration grew from eight in-scope rules to twenty-five and the score fell. On 2026-09-10 three more were declared, one per record family: each record is selected only on its exact `.v0` schema. The observation one is the rule that makes the triple's schema leg unreachable. All three survive, which takes the score to the current value shown at the start of this row. All six rules the corpus does isolate are now declared, and all six are killed. Measured on the surface the corpus itself declares normative, after a lock and a HEAD check closed a cross-run contamination. See below. **This is the corpus carrying our live reproduction request ([#1840](https://github.com/Rul1an/assay/issues/1840))** |
| `rfc8785-canonicalization` | batch (test-names) | **4 of 4 in scope (100.0%), 0 survivors.** The denominator is exactly four author-declared rules in the pinned `serde_jcs` dependency: UTF-16 object-key ordering, ECMAScript-compatible number serialization, section 3.2.2.2 Unicode string emission, and section 3.2.2.2 solidus non-escaping. The wrapper swap remains a separate control. This is not RFC certification, corpus coverage, or full implementation adequacy |
...
**Five measured, zero control-only, one not measurable.** Stated rather than
...
| `schema` | decided earlier, at Stage 2 — under v0 selection only `.v0` payloads ever enter `observations`; that selection rule is itself undiscriminated (declared mutant 49) |
...
The generated values above come directly from [`adequacy/results.json`](adequacy/results.json) through
[`adequacy/project_published_numbers.py`](adequacy/project_published_numbers.py). Historical and editorial number-shaped prose is listed below with its reason; it is not presented as a current measurement.

```json
{
  "not_derived": [
    {"token": "100%", "reason": "the adequacy bar itself, not a result"},
    {"token": "4 of 4", "reason": "the superseded first mcp-jsonrpc-id score"},
    {"token": "4 of 5", "reason": "the superseded first observed-effect score"},
    {"token": "30 of 30", "reason": "the superseded first rge-bench score"},
    {"token": "50%", "reason": "the superseded privileged-action score over eight declared rules"},
    {"token": "6 of 8", "reason": "the contaminated run that motivated the exclusive lock"},
    {"token": "75.0%", "reason": "the percentage from that contaminated run"},
    {"token": "4 of 8", "reason": "the clean rerun of that superseded declaration"},
    {"token": "8 of 31", "reason": "the RFC 8785 control divergence count, not a mutation score"},
    {"token": "one not measurable", "reason": "mcp-era-parity-v0 has no per-vector verdict and therefore no measurable row"}
  ]
}
```
