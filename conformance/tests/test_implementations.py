...
class AuthorshipProjectionPipelineAndLegacyContracts(unittest.TestCase):
    ...
    def test_capture_projection_drop_mutation_is_caught(self) -> None:
        def dropped(_val: object) -> dict:
            return {}

        auth = dict(HUMAN_AUTHORSHIP)
        with mock.patch.object(self.module, "project_capture_authorship", dropped):
            with self.assertRaises(AssertionError):
                self.assertEqual(self.module.project_capture_authorship(auth), auth)

    def test_capture_projection_rewrite_mutation_is_caught(self) -> None:
        def rewritten(val: object) -> dict:
            res = dict(self.module._validate_authorship(val, "test"))
            res["kind"] = "agent-generated"
            return res

        auth = dict(HUMAN_AUTHORSHIP)
        with mock.patch.object(self.module, "project_capture_authorship", rewritten):
            with self.assertRaises(AssertionError):
                self.assertEqual(self.module.project_capture_authorship(auth), auth)

    def test_run_projection_drop_mutation_is_caught(self) -> None:
        def dropped(_val: object) -> dict | None:
            return None

        auth = dict(ASSISTED_AUTHORSHIP)
        with mock.patch.object(self.module, "project_run_authorship", dropped):
            with self.assertRaises(AssertionError):
                self.assertEqual(self.module.project_run_authorship(auth), auth)

    def test_run_projection_rewrite_mutation_is_caught(self) -> None:
        def rewritten(val: object) -> dict | None:
            if val is None:
                return None
            res = dict(self.module._validate_authorship(val, "test"))
            if "model" in res:
                res["model"] = "normalized-model"
            return res

        auth = dict(ASSISTED_AUTHORSHIP)
        with mock.patch.object(self.module, "project_run_authorship", rewritten):
            with self.assertRaises(AssertionError):
                self.assertEqual(self.module.project_run_authorship(auth), auth)

    def test_public_projection_drop_mutation_is_caught(self) -> None:
        def dropped(_val: object) -> dict | None:
            return None

        auth = dict(GENERATED_AUTHORSHIP)
        with mock.patch.object(self.module, "project_public_authorship", dropped):
            with self.assertRaises(AssertionError):
                self.assertEqual(self.module.project_public_authorship(auth), auth)

    def test_public_projection_rewrite_mutation_is_caught(self) -> None:
        def rewritten(val: object) -> dict | None:
            if val is None:
                return None
            res = dict(self.module._validate_authorship(val, "test"))
            if "prompt_strategy" in res:
                res["prompt_strategy"] = "rewritten-strategy"
            return res

        auth = dict(GENERATED_AUTHORSHIP)
        with mock.patch.object(self.module, "project_public_authorship", rewritten):
            with self.assertRaises(AssertionError):
                self.assertEqual(self.module.project_public_authorship(auth), auth)

    def test_legacy_inferred_author_mutation_is_caught(self) -> None:
        def inferred(val: object) -> dict | None:
            if val is None:
                return {"kind": "human"}
            return self.module.project_run_authorship(val)

        with mock.patch.object(self.module, "project_run_authorship", inferred):
            with self.assertRaises(AssertionError):
                self.assertIsNone(self.module.project_run_authorship(None))

    def test_legacy_public_inferred_author_mutation_is_caught(self) -> None:
        def inferred(val: object) -> dict | None:
            if val is None:
                return {"kind": "human"}
            return self.module.project_public_authorship(val)

        with mock.patch.object(self.module, "project_public_authorship", inferred):
            with self.assertRaises(AssertionError):
                self.assertIsNone(self.module.project_public_authorship(None))
