...
profile's rules, and even those only at the points these vectors probe. **Twenty-three further rules
that v0.md promises are not discriminated by any vector here**, so an implementation can omit them and
still reproduce all fourteen expected outcomes. Two of the twenty-three cannot be closed by writing a
better vector at all, and one cannot be isolated by any bundle that could exist.

Five of twenty-eight is 17.9%. The run and the expectations are unchanged and the exercise is still
worth doing. What changes is the claim that can be made afterwards, by the implementer and by us, and
the report template now carries a field for it. [ERRATA.md](ERRATA.md) names all twenty-eight measured
