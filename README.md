...
cargo install assay-cli --version 6.1.1 --locked

python3 examples/mcp-quickstart/run.py
```

For v6.1.1, run the last command from a source checkout or an extracted published CLI archive.
...
- Published v6.1.1 CLI archives cover Linux x86_64/arm64, macOS x86_64/arm64, and Windows x86_64. The Python wheels cover CPython 3.12 on macOS x86_64/arm64 and Linux x86_64; other interpreters and platforms are not claimed.
...
Current release: [`v6.1.1`](https://github.com/Rul1an/assay/releases/tag/v6.1.1). [CHANGELOG.md](CHANGELOG.md) and release notes remain the authority for released behavior; merged changes after the tag are `Unreleased`, and crates.io publication is separate from merge state.
