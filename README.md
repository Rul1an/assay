...
`trust-basis.json` emits a fixed claim set (examples: `bundle_verified`, `delegation_context_visible`, `authorization_context_visible`, `containment_degradation_observed`, …). It is **not** a scalar trust score. The Trust Card is a deterministic render of the same claim rows plus frozen non-goals.
