"""Map frozen AGT audit NDJSON into Assay-shaped placeholder envelopes."""
...
PLACEHOLDER_EVENT_TYPE = "example.placeholder.agt-policy-decision"
PLACEHOLDER_SOURCE = "urn:example:assay:external:agt:mcp-trust-proxy"
PLACEHOLDER_PRODUCER = "assay-example"
PLACEHOLDER_PRODUCER_VERSION = "0.1.0"
PLACEHOLDER_GIT = "sample"
EXTERNAL_SCHEMA = "agt.mcp-trust-proxy.audit.v1"
REQUIRED_KEYS = ("allowed", "tool", "agent_did", "reason", "timestamp")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Map AGT audit NDJSON into Assay-shaped placeholder envelopes."
    )
    parser.add_argument("input", type=Path, help="AGT NDJSON export to read.")
    parser.add_argument(
        "--output",
        type=Path,
        required=True,
    ...
    parser.add_argument(
        "--assay-run-id",
        default=None,
        help="Optional Assay run id override. Defaults to import-agt-<stem>.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
