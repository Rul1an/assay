"""Map a frozen Microsoft Agent Framework trace export into Assay-shaped placeholder envelopes."""
...
PLACEHOLDER_EVENT_TYPE = "example.placeholder.microsoft-agent-framework-trace-span"
PLACEHOLDER_SOURCE = "urn:example:assay:external:microsoft-agent-framework:trace"
PLACEHOLDER_PRODUCER = "assay-example"
PLACEHOLDER_PRODUCER_VERSION = "0.1.0"
PLACEHOLDER_GIT = "sample"
EXTERNAL_SCHEMA = "microsoft.agent-framework.trace.export.v1"
REQUIRED_KEYS = (
    "schema",
    "framework",
    "surface",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Map Microsoft Agent Framework NDJSON trace export into Assay-shaped placeholder envelopes."
    )
    parser.add_argument("input", type=Path, help="Microsoft Agent Framework NDJSON export to read.")
    parser.add_argument(
        "--output",
        type=Path,
        required=True,
    ...
    parser.add_argument(
        "--assay-run-id",
        default=None,
        help="Optional Assay run id override. Defaults to import-maf-<stem>.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def _validate_record(record: dict[str, Any], line_number: int) -> None:
    ...
    if record["framework"] != "microsoft-agent-framework":
        raise ValueError(f"line {line_number}: framework must be microsoft-agent-framework")
    if record["surface"] != "exported_opentelemetry_trace":
...
def _normalized_record(record: dict[str, Any], line_number: int) -> dict[str, Any]:
    ...
    try:
        normalized["start_time"] = _parse_rfc3339_utc(str(record["start_time"]))
        normalized["end_time"] = _parse_rfc3339_utc(str(record["end_time"]))
    except ValueError as exc:
...
def _build_event(
    ...
    data = {
        "external_system": "microsoft-agent-framework",
        "external_surface": "exported-opentelemetry-trace",
        "external_schema": EXTERNAL_SCHEMA,
        "observed_upstream_time": normalized["start_time"],
        "observed": record,
    }
    event = {
        "specversion": "1.0",
...
def main() -> int:
    ...
    assay_run_id = args.assay_run_id or f"import-maf-{args.input.stem}"
