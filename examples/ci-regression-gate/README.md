...
This example demonstrates how to use Verdict to prevent regressions in your CI pipeline without making live LLM calls.

## How it works
1.  **Baseline Export**: You record a "golden" trace (`traces/main.jsonl`) from your main branch and export a `baseline.json`.
2.  **PR Check**: When a PR is opened, you record a new trace (`traces/pr_bad.jsonl`) and Verdict compares it against the baseline.
3.  **Strict Gating**: If the new trace degrades performance (e.g., changes "hello" to "goodbye"), Verdict fails the build.
...
*   `verdict-baseline-export.yml`: Updates the baseline on pushes to `main`.
*   `verdict-pr-gate.yml`: Checks PRs against the baseline from the base branch.
